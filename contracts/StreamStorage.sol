...
contract DealClient is AxelarExecutable {
    ...
    mapping(uint256 => Status) public streamStorageState;
    mapping(bytes => uint256) public pieceToEventId;
        ...
        returns (bytes32)
    {
        ...
        streamStorageState[eventId] = Status.RequestSubmitted;
        pieceToEventId[deal.piece_cid] = eventId;
        // writes the proposal metadata to the event log

        require(msg.value > 0, "Cross-chain gas required");
    ...
    function dealNotify(bytes memory params) internal {
        ...
        streamStorageState[pieceToEventId[proposal.piece_cid.data]] = Status
            .DealPublished;
    }
    ...
    function updateActivationStatus(bytes memory pieceCid) public {
        ...
        uint256 eventId = pieceToEventId[pieceCid];
        ...
        if (CommonTypes.ChainEpoch.unwrap(ret.terminated) > 0) {
            ...
            streamStorageState[eventId] = Status.DealTerminated;
        } else if (CommonTypes.ChainEpoch.unwrap(ret.activated) > 0) {
            pieceStatus[pieceCid] = Status.DealActivated;
            streamStorageState[eventId] = Status.DealActivated;
        }
    }
